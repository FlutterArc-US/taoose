...
class PostController extends GetxController {
  ...
  getFollowingPosts() async {
    ...
    for (int i = 0; i < loops; i++) {
      var ref = docFetch.doc(followings[i].toString()).collection('one').orderBy("timestamp").limit(2);
      var docu = await ref.get();

      posts.value.addAll(docu.docs.map((doc) => doc.data()).toList());

      ref = docFetch.doc(followings[i].toString()).collection('two').orderBy("timestamp").limit(2);
      docu = await ref.get();

      posts.value.addAll(docu.docs.map((doc) => doc.data()).toList());

      ref = docFetch.doc(followings[i].toString()).collection('three').orderBy("timestamp").limit(2);
      docu = await ref.get();
